...
const Skills = ({ isDarkMode }) => {
  ...
  const skills = {
    ...
    "Tools & Technologies": [
      ...
      { name: "BullMQ", icon: <FaAws /> },
      { name: "Docker", icon: <FaDocker /> },
      { name: "Firebase Push/Dynamic Links", icon: <SiFirebase /> },
    ],
    "Payment Gateways": [
      { name: "Cashfree", icon: <FaAws /> },
      { name: "Stripe", icon: <SiStripe /> },
      { name: "Razorpay", icon: <SiRazorpay /> },
    ],
    "Security & Authentication": [
      { name: "RBAC", icon: <FaAws /> },
      { name: "API Encryption", icon: <FaAws /> },
      { name: "OAuth", icon: <FaAws /> },
      { name: "JWT", icon: <FaAws /> },
    ],
    ...
    "Version Control & Collaboration": [
      { name: "Git", icon: <FaGitAlt /> },
      { name: "GitHub", icon: <FaGitAlt /> },
      { name: "Agile Methodologies", icon: <FaAws /> },
    ],
  };
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        >
          {Object.entries(skills).map(([category, skillList], index) => (
            <motion.div
              key={index}
              className={`p-6 rounded-2xl shadow-lg transition-all duration-300 ${isDarkMode ? "bg-gray-800" : "bg-white"}`}
              variants={item}
            >
              ...
              <div className="flex flex-wrap gap-2">
                {skillList.map((skill, i) => (
                  <motion.span
                    key={i}
                    className={`px-3 py-2 rounded-full text-sm border transition-colors flex items-center gap-2 ${isDarkMode
